...
const defaultConfig = {
  ...
  httpPath: '/mcp',
  verbose: false,
  // Cloudflare Access JWT validation for HTTP mode (see access-jwt.mjs). Both
  // must be set to enable enforcement; otherwise the origin trusts any caller.
  accessTeamDomain: '',
  accessAud: ''
};
...
export function createServer(userConfig = {}) {
  ...
  const config = {
    ...
    accessTeamDomain: userConfig.accessTeamDomain || process.env.CF_ACCESS_TEAM_DOMAIN || defaultConfig.accessTeamDomain,
    accessAud: userConfig.accessAud || process.env.CF_ACCESS_AUD || defaultConfig.accessAud
  };
  ...
  async function startHttp() {
    ...
    // Validate the Cloudflare Access JWT before any MCP route runs. No-op (with a
    // startup warning) unless accessTeamDomain + accessAud are configured.
    app.use(config.httpPath, createAccessJwtMiddleware({
